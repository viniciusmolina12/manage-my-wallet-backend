...
describe('MongoDB Item Repository tests', () => {
    ...
    it('should update a bill', async () => {
        ...
        const oldBill = new Bill('any_id', 'any_name', [oldBillItem], new Date(), 'any_description' )
        await BillModel.create({_id: oldBill.id, name: oldBill.name, description: oldBill.description, createdDate: new Date(), items: [oldBillItem]});
    ...
    it('should find all bills', async () => {
        ...
        await BillModel.create({_id: bill1.id, name: bill1.name, description: bill1.description, createdDate: new Date(), items: [billItem]});
        const bill2 = new Category('any_hash_id_2', 'Category 2', 'Description 2');
        await BillModel.create({_id: bill2.id, name: bill2.name, description: bill2.description, createdDate: new Date(), items: [billItem, billItem]});
