...
export default class MongoDbBillRepository implements BillRepository { 
    async create(entity: Bill): Promise<void> {
        await BillModel.create({_id: entity.id, name: entity.name, items: entity.items, createdDate: entity.createdDate, description: entity.description});
    }
    async update(entity: Bill): Promise<void> {
        await BillModel.findOneAndUpdate({ _id: entity.id }, { name: entity.name, items: entity.items, description: entity.description });
    }

    async find(id: string): Promise<Bill | null> {
        const bill = await BillModel.findOne({ _id: id }).populate('items.itemId');
        if(!bill) return null
        const billItems = bill.items.map(item => new BillItem(item._id, item._id, item.price, item.quantity))
        return new Bill(bill._id.toString(), bill.name, billItems, bill.createdDate, bill?.description)
    }
