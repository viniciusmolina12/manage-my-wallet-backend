...
describe('Category entity tests', () => {
   it('Should create a category', () => {
      const category = new Category('123', 'Category', '123', 'Description');
   ...
   it('Should throw an error if id is missing', () => {
      expect(() => {
         const category = new Category('', 'Category', '123', 'Description');
      }).toThrow(new EntityError('category: Id is required, '));
   });
   it('Should throw an error if name is missing', () => {
      expect(() => {
         const category = new Category('123', '', '123', 'Description');
