...
describe('Update bill usecase', () => {
    ...
    it('should throw an error if name is empty', async () => {
        ...
        const sut = new UpdateBillUseCase(mockRepository);
        await expect(sut.execute({ ...input, name: ''} )).rejects.toThrow(new EntityError('bill: Name is required, '));
    } )
    ...
    it('should throw an error if bill does not exist', async () => {
        ...
        const sut = new UpdateBillUseCase(mockRepository);
        await expect(sut.execute({ ...input, name: ''} )).rejects.toThrow(new Error('Bill not exists'));
    } )
})