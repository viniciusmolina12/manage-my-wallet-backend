...
const user = new UserWithResetToken('any_id', 'any_name', 'any_email@mail.com', 'any_password');
...
describe('Reset password user usecase tests', () => {
    ...
    it('should throw if user not found', async () => {
        ...
        const sut = new ResetPasswordUserUseCase(mockRepository, new EncryptStub());
        await expect(sut.execute({token: 'any_token', password: 'any_other_password'})).rejects.toThrow(new EntityError('User not found'));
    })
    ...
    it('should throw if token expired', async () => {
        ...
        mockRepository.findUserByResetPasswordToken.mockReturnValueOnce(user);
        const sut = new ResetPasswordUserUseCase(mockRepository, new EncryptStub());
        await expect(sut.execute({token: 'any_token', password: 'any_other_password'})).rejects.toThrow(new EntityError('Token expired'));
    })
    ...
    it('should throw if token invalid', async () => {
        ...
        mockRepository.findUserByResetPasswordToken.mockReturnValueOnce(user);
        const sut = new ResetPasswordUserUseCase(mockRepository, new EncryptStub());
        await expect(sut.execute({token: 'invalid_token', password: 'any_other_password'})).rejects.toThrow(new EntityError('Token invalid'));
    })
})
