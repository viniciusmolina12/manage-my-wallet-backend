...
describe('Update user usecase tests', () => {
    ...
    it('should throw an error if email already exists', async () => {
        const sut = new UpdateUserUseCase(mockRepository)
        mockRepository.find.mockReturnValueOnce(new User('any_id', 'any_name', 'any_email@mail.com', 'any_password'));
        mockRepository.search.mockReturnValueOnce(Promise.resolve([{id: 'any_id', name: 'any_name', email: 'any_other_email@mail.com', userName: 'any_username', password: 'encrypted_password'}]));
